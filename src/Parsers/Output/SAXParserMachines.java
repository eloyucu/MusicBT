...
public class SAXParserMachines extends DefaultHandler
{
	private boolean machineFlag = false;
	private int type = 0;
	private int contador = 0;
	private ResultMachines result;
	
	public SAXParserMachines(String fichero) 
	{  
		  result = new ResultMachines();
   ...
   {  
	   ...
	   if(!str.startsWith("\n"))
	   {
		   if(!machineFlag)
		   {
			   result.addNombreMaq(str,contador);
			   
			   result.addNombre(null,contador);	  
		   ...
		   else
		   {
			   switch(type)
			   {
			   		case 0: result.addNombre(str,contador);	   type=1; break;
   ...
   public ResultMachines getResult()
   {
	   return result;
   }
}
